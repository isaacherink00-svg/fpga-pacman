...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	u8 writebuffer[2] = {reg + 2, val};
	int returncode = XSpi_Transfer(&SpiInstance, writebuffer, NULL, 2);
	if (returncode != 0){
		xil_printf("Error with slave select");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int returncode = XSpi_Transfer(&SpiInstance, writebuffer, NULL, nbytes +1); // Write each byte
	if (returncode != 0){
			xil_printf("Error with slave select");
		}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
...
BYTE MAXreg_rd(BYTE reg) {
	u8 writebuffer[2] = {reg + 0, 0};
	u8 readbuffer[2];
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	int returncode = XSpi_Transfer(&SpiInstance, writebuffer, readbuffer, 2);
	if (returncode != 0){
		xil_printf("Error with slave select");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	writebuffer[0] = reg + 0;
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	int returncode = XSpi_Transfer(&SpiInstance, writebuffer, readbuffer, nbytes + 1);
	for (int n = 0; n < nbytes; n++) {
